...
def collect_data():
	...
	while i<200:
		...
		substr_time = "ms"
		while j<3:
			data = subprocess.run(["./0_added-bgv", str(x1), str(x2), str(x3), str(securityParams[j])],  capture_output=True)
			data = data.stdout.decode('utf-8')
			data = data.split(" ")
			data = ' '.join(data).split()
			bgvkeygenRtime = data[1].replace(substr_time, "")
			bgvencryptTime = data[3].replace(substr_time, "")
			bgvMultTime = data[5].replace(substr_time, "")
			bgvAddTime = data[7].replace(substr_time, "")
			bgvDecryptTime = data[9].replace(substr_time, "")
			
			data = subprocess.run(["./0_added-bfv", str(x1), str(x2), str(x3), str(securityParams[j])],  capture_output=True)
			data = data.stdout.decode('utf-8')
			data = data.split(" ")
			data = ' '.join(data).split()
			
			bfvkeygenRtime = data[1].replace(substr_time, "")
			bfvencryptTime = data[3].replace(substr_time, "")
			bfvMultTime = data[5].replace(substr_time, "")
			bfvAddTime = data[7].replace(substr_time, "")
			bfvDecryptTime = data[9].replace(substr_time, "")
			
			data = subprocess.run(["./0_added-ckks", str(x1), str(x2), str(x3), str(securityParams[j])],  capture_output=True)
			data = data.stdout.decode('utf-8')
			data = data.split(" ")
			data = ' '.join(data).split()

			ckkskeygenRtime = data[1].replace(substr_time, "")
			ckksencryptTime = data[3].replace(substr_time, "")
			ckksMultTime = data[5].replace(substr_time, "")
			ckksAddTime = data[7].replace(substr_time, "")
			ckksDecryptTime = data[9].replace(substr_time, "")
	...
	# name of csv file 
	filename = "openfhe_generated_data.csv"
	# writing to csv file
...
def averaging_data():
	securityParams = ['8192','16384','65536']
	header = ['#', 'securityParam', 'x1','x2','x3',
		   'bgvkeygenRtime','bgvencryptTime', 'bgvMultTime', 'bgvAddTime','bgvDecryptTime',
		   'bfvkeygenRtime','bfvencryptTime', 'bfvMultTime', 'bfvAddTime','bfvDecryptTime',
		   'ckkskeygenRtime','ckksencryptTime', 'ckksMultTime', 'ckksAddTime','ckksDecryptTime',
...
def graph_separately_display():
	...
	df = pd.read_csv("openfhe_averages.csv")
