...
def collect_data():
	...
	while i<200:
		...
		substr_time = "ms"
		while j<2:
			data = subprocess.run(["./00_bgv", str(x1), str(x2), str(x3), str(securityParams[j])],  capture_output=True)
			...
			data = subprocess.run(["./00_ckks", str(x1), str(x2), str(x3), str(securityParams[j])],  capture_output=True)
			data = data.stdout.decode('utf-8')
			data = data.split(" ")
			data = ' '.join(data).split()
	...
	# name of csv file 
	filename = "helib_generated_data.csv"
	# writing to csv file
...
def graph_separately_display():
	...
	df = pd.read_csv("helib_averages.csv")
